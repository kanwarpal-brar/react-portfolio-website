...
// Deliberately smaller than a ring node (see RING_SLOTS-derived .snode size
// in style.css, ~12ch/3.2rem) so the hub reads as the smallest thing on the
// board, not a peer of the ring — the wheel is what carries visual weight.
const SECTION_CARD_MAX = { w: 100, h: 58 };
...
const SECTION_CARD_MIN = { w: 80, h: 46 };
...
export function applyGeometry() {
	...
	let collapsed =
		gr.height < COLLAPSE_MIN_HEIGHT ||
		gr.width < COLLAPSE_MIN_WIDTH ||
		gr.width * gr.height < COLLAPSE_MIN_AREA;
	...
	if (!collapsed && ringNodes.length) {
		if (wheelVisible) {
			...
			const f = {
				halfW: Math.min(minHalfW, maxHalfW),
				halfH: Math.min(minHalfH, maxHalfH),
			};
	...
	if (!collapsed && cardW && cardH) {
		if (bandH)
			cardH = fitCardHeight(
				...
				cardW,
				bandH,
				wheelVisible ? SECTION_CARD_MIN.h : CARD_MIN.h,
			);
...
function drawSpokes(graph, wheel, gr, cx, cy, cardHalf, ringNodes, state) {
	...
	for (const l of lines) {
		...
		if (l.active)
			cls += l.type === "ring" ? " line-ring-active" : " line-wheel-active";
