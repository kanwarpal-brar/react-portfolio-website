...
export function wheelItemHTML(id) {
  ...
  if (c.kind === 'project') {
    const tags = c.data.tags.slice(0, 3).map((t) => esc(t)).join(' · ');
    return (
...
function childDetailHTML(childId) {
  ...
  if (c.kind === 'project') return `<div class="proj-grid single">${projBoxHTML(c.data, { linked: false })}</div>`;
...
export function cardHTML(state) {
  ...
  if (state.view === 'child') {
    return breadcrumbHTML(state.section, state.child) + childDetailHTML(state.child);
  }
  return '';
}
...
export function pageTitle(state) {
  ...
  if (state.view === 'child') return `${state.child} — ${state.section} — ${identity.name}`;
  const titles = {
